package cn.zmy.common.base.widget;
...
import cn.zmy.common.base.widget.internal.CircleImageView;
import cn.zmy.common.base.widget.internal.MaterialProgressDrawable;


/**
 * Created by zmy on 2016/8/5 0005.
 */
public class SwipeTopBottomLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild
{
    //旋转View的默认背景
    ...
    public void requestDisallowInterceptTouchEvent(boolean b)
    {
        ...
        // isn't stolen
        if ((Build.VERSION.SDK_INT < 21 && mainChild instanceof AbsListView) || (mainChild != null && !ViewCompat.isNestedScrollingEnabled(mainChild)))
        {
            // Nope.
        }
        else
    ...
    public void setRefreshing(boolean refreshing)
    {
        ...
        if (refreshing)
        {
            ...
            int offset = executeOffset - circleImageViewTop.getTop() - circleImageViewTop.getHeight();
            ViewCompat.offsetTopAndBottom(circleImageViewTop, offset);
    ...
    class HasScaleUpTopListener implements Animation.AnimationListener
    {
        ...
        public void onAnimationEnd(Animation animation)
        {
            //NOTE：实际项目中发现当在Fragment中使用时，由于hide操作会导致setRefresh方法中的偏移代码无效
            //这里需要确保已经偏移到正确的位置
            int correctTop = executeOffset - circleImageViewTop.getHeight();
            if (circleImageViewTop.getTop() != correctTop)
            {
                ViewCompat.offsetTopAndBottom(circleImageViewTop,correctTop - circleImageViewTop.getTop());
            }
    ...
    class HasScaleUpBottomListener implements Animation.AnimationListener
    {
        ...
        public void onAnimationEnd(Animation animation)
        {
            //NOTE：实际项目中发现当在Fragment中使用时，由于hide操作会导致setRefresh方法中的偏移代码无效
            //这里需要确保已经偏移到正确的位置
            int correctTop = getHeight() - executeOffset;
            if (circleImageViewBottom.getTop() != correctTop)
            {
                ViewCompat.offsetTopAndBottom(circleImageViewBottom,correctTop - circleImageViewBottom.getTop());
            }
            isLoadingMore = true;
            progressDrawableBottom.setAlpha(255);
            progressDrawableBottom.start();
