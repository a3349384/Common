...
/**
 * Created by zmy on 2016/6/6 0006.
 */
public class BitmapUtil
{
    public static int getMaxTextureSize(Context context)
    {
        ...
        Bitmap bitmap;
        int screen = Math.max(ScreenUtil.getScreenWidth(context),ScreenUtil.getScreenHeight(context));
        if (screen <= 2048)
        {
            return 2048;
        }

        //应该还不存在大于4K分辨率的屏幕，后面就懒得判断了
        return 4096;
    }

    @TargetApi(17)
    public static int getEGL14MaxTextureSize()
    {
        try
        {
    ...
    /**
     * Apply a blur to a Bitmap by Algorithm
     * @param sentBitmap
